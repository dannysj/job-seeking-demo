...
class AccountNotification extends React.Component {
  ...
  retrieveMessage(){
    axios.post('/api/get_system_notifications', {access_token: this.props.user.access_token}).then(res => {
      if(res.data.code === 0){
    ...
    axios.post('/api/read_system_notification', {access_token: this.props.user.access_token}).then(res => {
      if(res.data.code === 0){
