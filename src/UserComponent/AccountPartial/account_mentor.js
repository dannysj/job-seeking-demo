...
class AccountMentor extends React.Component {
  ...
  updateInfo(){
    let handler = this;
    axios.post('/api/get_rel_mentors', {access_token: this.props.user.access_token}).then(res => {
      if(res.data.code === 0){
  ...
  handleConfirm(mid) {
    let handler = this;
    axios.post('/api/mentee_confirm', {access_token: this.props.user.access_token, mid: mid}).then(res => {
      if(res.data.code === 0){
        handler.updateInfo();
      }
      else{
